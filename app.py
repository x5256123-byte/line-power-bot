...
def process_all_records(uid, token, r_list):
    ...
    reply_msg = f"已載入 {site_name}，發現設備：\n{summary if summary else '無匹配型號'}\n\n輸入「計算」顯示報告，或「型號 數量」追加。"
    line_bot_api.reply_message(token, TextSendMessage(text=reply_msg))
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 8080)))
        sheet = client.open_by_key("172To-4ENLnZutCsPP7qXCpXNANo4A5YNcyuRadHUGzg").worksheet("工作表1")
        all_values = sheet.get_all_values()
        return [{"台號": str(r[0]).strip(), "台名": str(r[1]).strip(), "模組型號": str(r[3]).strip(), "(模組位置 / 光接點)": str(r[5]).strip()} for r in all_values[2:] if len(r) >= 6]
    except Exception as e:
        print(f"DEBUG: 連線異常: {e}")
        return []
