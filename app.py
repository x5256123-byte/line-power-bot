        ...
        with open(CSV_FILE_PATH, encoding='utf-8-sig') as f:
            for r in csv.DictReader(f):
                if msg in str(r.get("台號", "")):
                    results.append({"name": f"{r['台名']}-{r['(模組位置 / 光接點)'].split('/')[-1]}", "equip": r.get("模組型號", "").split('_')[0]})
        
        if not results:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="查無此站，請重試或輸入 0。"))
        elif len(results) > 1:
